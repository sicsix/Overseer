...
export class CreepExporter {
    buf;
    bodyPartLookup = new Map([[MOVE, 1], [WORK, 2], [CARRY, 3], [ATTACK, 4], [RANGED_ATTACK, 5], [TOUGH, 6], [HEAL, 7]]);
        ...
        for (let i = 0; i < creeps.length; i++) {
            ...
            if (creep.body.length > 1) {
                for (let j = 0; j < creep.body.length; j += 2) {
                    ...
                    this.buf.WriteVal(this.bodyPartLookup.get(body1.type) << 24 | body1.hits << 16 | this.bodyPartLookup.get(body2.type) << 8 | body2.hits);
                }
            }
            if (creep.body.length % 2 == 1) {
                let body = creep.body[creep.body.length - 1];
                this.buf.WriteVal(this.bodyPartLookup.get(body.type) << 24 | body.hits << 16);
            }
...
//# sourceMappingURL=creepExporter.js.map