...
export class Creep extends GameObject {
    ...
    withdraw(target, resourceType, amount) {
        ...
        const emptySpace = SystemStore.roomObjectsData[this.id].storeCapacity -
    ...
    transfer(target, resourceType, amount) {
        ...
        if (!SystemStore.roomObjectsData[this.id].store ||
        ...
        if (!SystemStore.roomObjectsData[target.id].store ||
    ...
    pickup(target) {
        ...
        if (sumObjectValues(SystemStore.roomObjectsData[this.id].store) >=
...
export class GameObject {
    ...
    [inspect](depth, options) {
        return (options.stylize(this.constructor.name, 'special') +
            ' ' +
            formatWithOptions({ colors: true, breakLength: 120, customInspect: false }, this.toJSON()));
    }
}
...
export class StructureSpawn extends OwnedStructure {
    ...
    spawnCreep(body) {
        ...
        const energyAvailable = Object.values(SystemStore.roomObjectsData)
            .filter(i => (i.user === SystemStore.roomObjectsData[this.id].user &&
            i.type == 'spawn') ||
            i.type == 'extension')
...
export function createConstructionSite(x, y, structurePrototype) {
    ...
    if (!checkConstructionSite(structurePrototype.prototype.constructor.name, x, y, Object.values(SystemStore.roomObjectsData), SystemStore.terrain, SystemStore.arenaSize)) {
        return returnError(C.ERR_INVALID_TARGET);
    }
    if (Object.values(SystemStore.roomObjects).filter(i => i.type === 'constructionSite').length +
...
export function getTime() {
    if (SystemStore.codeCreatedAt > breakingChanges.getTime) {
        throw new Error('utils::getTime function is deprecated, please use utils::getTicks instead');
    }
    return SystemStore.time;
}
...
export function searchPath(origin, goal, options) {
    ...
    // Invoke native code
    const ret = SystemStore.nativeMod.search(toWorldPosition(origin), goals, cb, plainCost, swampCost, 1, maxOps, maxCost, flee, heuristicWeight);
    if (ret === undefined) {
...
export const utils = {};
export const visual = {};
//# sourceMappingURL=exportedConstants.js.map